  ...
  return (
    <div className="flex-1 flex flex-col pt-8 pb-12 px-6 h-full grow animate-in fade-in slide-in-from-right-4 duration-300">
      <div className="mb-6 shrink-0">
        ...
        <h2 className="font-serif text-3xl font-bold text-foreground mb-2">Destination & Vibe</h2>
        <p className="text-background0 text-base">Where are we heading?</p>
      </div>
          ...
          {Object.entries(config.regions).map(([regionId, region]) => (
            ...
            >
              ...
              <div className="flex-1">
                <h3 className={`font-bold text-lg ${data.region === regionId ? 'text-foreground' : 'text-stone-700'}`}>{region.title}</h3>
                <p className="text-sm text-background0">{region.desc}</p>
              </div>
            </button>
          ))}
        </div>
